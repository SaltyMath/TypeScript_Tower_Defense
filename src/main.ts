...
class Tower {
  ...
  update(delta: number, enemies: Enemy[], projectiles: Projectile[]) {
    ...
    projectiles.push(
      new Projectile({ x: this.position.x, y: this.position.y }, target)
    );

    this.cooldown = this.fireRate;
  }

  getDirectionRow(target: Vec2) {
    const dx = target.x - this.position.x;
    const dy = target.y - this.position.y;
    ...
    if (degrees >= -157.5 && degrees < -112.5) return 7; // N
    if (degrees >= -112.5 && degrees < -67.5) return 6; // NE
    if (degrees >= -67.5 && degrees < -22.5) return 5; // E
    if (degrees >= -22.5 && degrees < 22.5) return 4; // SE
    if (degrees >= 22.5 && degrees < 67.5) return 3; // S
    if (degrees >= 67.5 && degrees < 112.5) return 2; // SO
    if (degrees >= 112.5 && degrees < 157.5) return 1; // O
...
class Game {
  ...
  handleClick(event: MouseEvent) {
    ...
    if (
      this.towers.some(
        (tower) => distance(tower.position, position) < GRID_SIZE * 0.75
      )
    ) {
      return;
    }
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    if (enemyCastle) {
      ...
      ctx.fillStyle = "#b33";
      ctx.fillRect(
        start.x - GRID_SIZE * 0.25,
        start.y - GRID_SIZE * 0.75,
        GRID_SIZE * 1.5,
        GRID_SIZE * 1.5
      );
    }
    ...
    if (allyCastle) {
      ...
      ctx.fillStyle = "#3b3";
      ctx.fillRect(
        end.x - GRID_SIZE * 1.25,
        end.y - GRID_SIZE * 0.75,
        GRID_SIZE * 1.5,
        GRID_SIZE * 1.5
      );
    }
